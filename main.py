...
def parse_text(text):
    ...
    # 5. Ambil Feeq
    fee_match = re.search(r"Feeq\s*:\s*([\d.]+)\s*", text, re.IGNORECASE)
    if fee_match:
        data["feeq"] = fee_match.group(1).strip()

    # 6. Ambil Agency Income
    agency_match = re.search(r"Agency Income\s*=\s*([\d.]+)\s*IDR", text, re.IGNORECASE)
    if agency_match:
        data["agency_income"] = agency_match.group(1).strip()

    # 7. Ambil Periods (jika ada isinya)
    period_match = re.search(r"Periods\s*=\s*(.*)", text)
    if period_match:
        data["periods"] = period_match.group(1).strip()

    # 8. Ambil Link Payment (jika ada isinya)
    link_match = re.search(r"Link Payment\s*:\s*(.*)", text, re.IGNORECASE)
    if link_match:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        "SALARY" in text.upper()
    ):  # Pakai .upper() biar aman kalau ada yang ngetik 'Salary'
